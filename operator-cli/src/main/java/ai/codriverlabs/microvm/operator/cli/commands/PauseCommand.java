...
@Command(name = "pause", description = "Pause a running MicroVM", mixinStandardHelpOptions = true)
public class PauseCommand implements Runnable {
    ...
    public void run() {
        try {
            ...
            vm.getSpec().setDesiredState(DesiredState.PAUSED);
            client.resources(MicroVM.class)
                .inNamespace(namespace)
                .resource(vm)
                .patch();

            System.out.printf("MicroVM \"%s\" pausing%n", name);
